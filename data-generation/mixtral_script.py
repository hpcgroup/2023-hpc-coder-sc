...
if ray.is_initialized():
    ray.shutdown()
llm = LLM(model="mistralai/Mixtral-8x7B-Instruct-v0.1",gpu_memory_utilization=0.95, max_num_seqs=1,swap_space=1,tensor_parallel_size=2)
print ("model loaded")
...
with alive_bar(total_samples) as bar:
    ...
    for element in seed_dataset:
      ...
      if(i > 50720):
        ...
        if(i % 100 == 0):
            with open('mixtral-outputs-2.json', 'w') as fp:
...
with open('mixtral-outputs-2.json', 'w') as fp:
    json.dump(results, fp)
datasets.Dataset.from_list(results).push_to_hub('hpcgroup/hpc-synthetic-mixtral', token='HF write token')
